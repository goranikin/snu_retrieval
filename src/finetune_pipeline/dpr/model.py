...
from base.retrieval import Retrieval
...
class Dpr(Retrieval):
    def __init__(self, device=None):
        ...
        self.paper_encoder = DPRContextEncoder.from_pretrained(
            "facebook/dpr-ctx_encoder-single-nq-base"
        ).to(self.device)
    ...
    def encode_query(self, query_text: str, no_grad: bool = True):
        if no_grad:
            with torch.no_grad():
    ...
    def encode_paper(self, title: str, abstract: str, no_grad: bool = True):
        if no_grad:
            with torch.no_grad():
