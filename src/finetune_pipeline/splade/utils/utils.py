...
def get_initialize_config(exp_dict: DictConfig, train=False):
    ...
    if train:
        ...
        OmegaConf.save(
            config=exp_dict,
            f=os.path.join(exp_dict.config.checkpoint_dir, "config.yaml"),
        )
        ...
        if config.pretrained_no_yamlconfig:
            ...
            model_training_config = OmegaConf.load(
                os.path.join(config["checkpoint_dir"], "config.yaml")
            )["config"]
        ...
        if "hf_training" in config:
            ...
            init_dict.model_type_or_dir_q = (
                os.path.join(config.checkpoint_dir, "model/query")
